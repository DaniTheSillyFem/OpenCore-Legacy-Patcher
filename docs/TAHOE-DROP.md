...
This affects not only macOS Tahoe, but macOS Ventura, Sonoma and Sequoia are confirmed to have the same issue. Thus an underlying problem with the MacBookAir8,x's firmware where it is not happy with OpenCorePkg.
...
This build includes both Legacy Metal and non-Metal patches for macOS Tahoe. Refer to the following links for more information about Legacy Metal and non-Metal support and their respective issues.
